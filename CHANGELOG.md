...
- **Release-age freshness honored with `trust_upstream_dates` (#748)** — under #742, `min_release_age` defers to quarantine, which holds on NORA's own clock, so a provably-old artifact was held as "new to this mirror" regardless of its release date. When `server.trust_upstream_dates` is set and the registry supplies a date, the quarantine now seeds first-seen from the trusted upstream release date — an artifact older than the TTL matures immediately and is served, while a fresh one is still held. Wired for PyPI (via the PEP 691 / PEP 700 `upload-time`), npm, Cargo, Go, NuGet, Conan and pub.dev (Terraform is a follow-up); Docker stays on NORA's own clock (digest-addressed).
